...
class XGBModeling:
    ...
    def predict(
        ...
        """
        data = data.copy()
        predictions = self.get_probability(model=model, pred_df=data)
    ...
    def run(self, data:pd.DataFrame):
        ...
        # Train model
        model, scoring, best_score = self.cv_grid_search(
            estimator=model,
            x=X_train,
            y=y_train,
        )
        ...
        train_prediction = self.predict(model=model, data=X_train)
        test_prediction = self.predict(model=model, data=X_test)

        report = self.model_evaluation(y_true=y_test, y_pred=pd.DataFrame(eval_preds))
        features = list(data.columns.values)
        features.remove("is_ckd")
        importance = self.feature_importance(model=model, data=X_train, features=features)
    ...
    def explainer_dashboard(self, data, model) -> None:
        X = data.drop(["is_ckd"], axis=1)#.values  # independant features
        y = data["is_ckd"]#.values  # dependant variable
...
if __name__ == "__main__":
    ...
    xgbm = XGBModeling()
    model = xgbm.run(data=data)
    xgbm.explainer_dashboard(data=data, model=model)
